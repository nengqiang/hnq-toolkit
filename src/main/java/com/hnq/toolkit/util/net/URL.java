...
public class URL implements Serializable {
    ...
    public boolean isLocalHost() {
        return NetUtils.isLocalHost(host) || getParameter(NetUtils.LOCALHOST_KEY, false);
    }

    public boolean isAnyHost() {
        return NetUtils.ANYHOST.equals(host) || getParameter(ANYHOST_KEY, false);
    }
    ...
    private void buildParameters(StringBuilder buf, boolean concat, String[] parameters) {
        if (getParameters() != null && getParameters().size() > 0) {
            List<String> includes = (parameters == null || parameters.length == 0 ? null
